...
def collate_fn(
    ...
    if target_type == "both":
        ...
        tgt_preproc = lambda x: (
            " ".join([x["target"][i] for i in sorted(x["contrib_indices"])]),
            " ".join(
                [s for i, s in enumerate(x["target"]) if s not in x["contrib_indices"]]
            ),
        )
        ...
        if target_type == "full":
            ...
            tgt_preproc = lambda x: " ".join(
                [x["target"][i] for i in sorted(x["contrib_indices"])]
            )
